...
class SYSTEM:
    def __init__(self):
        ...
        self.combobox_board = ttk.Combobox(
            self.root, textvariable=StringVar(), width=20, background=BACKGROUND
        )
        ...
        # Turn
        self.label_firstturn = Label(
            self.root, text="First Turn:", background=BACKGROUND
        )
        self.label_firstturn.place(x=300, y=10)

        self.combobox_firstturn = ttk.Combobox(
            self.root, textvariable=StringVar(), width=8, background=BACKGROUND
        )
        ...
        # Start Game
        self.button_startgame = Button(
            self.root,
            text="Start Game!",
            width=10,
            fg="grey20",
            highlightbackground=BACKGROUND,
            command=self.set_new_board,
        )
        self.button_startgame.place(x=480, y=8)

        # Canvas
        self.board = Canvas(
            self.root, width=CANVAS_SIZE, height=CANVAS_SIZE, background="white"
        )
        ...
        self.label_userscore2 = Label(
            self.root, text=self.score[0], background=BACKGROUND
        )
        self.label_userscore2.place(x=score_x + 45, y=score_y + 30)

        self.label_machinescore1 = Label(
            self.root, text="MACHINE:", background=BACKGROUND
        )
        self.label_machinescore1.place(x=score_x, y=score_y + 55)
        self.label_machinescore2 = Label(
            self.root, text=self.score[1], background=BACKGROUND
        )
        ...
        self.label_user = Label(
            self.root, text="[ USER ]", background=BACKGROUND, fg=USER_COLOR
        )
        ...
        self.start_x = Entry(
            self.root, textvariable=IntVar(), width=3, highlightbackground=BACKGROUND
        )
        ...
        self.label_machine = Label(
            self.root, text="[ MACHINE ]", background=BACKGROUND, fg=MACHINE_COLOR
        )
        self.label_machine.place(x=machine_x, y=machine_y)

        self.button_machinego = Button(
            self.root,
            text="Go!",
            width=10,
            fg="grey20",
        ...
        self.label_result = Label(
            self.root, text="The game is ongoing!!", background=BACKGROUND
        )
    ...
    def set_new_board(self):
        ...
        self.location = [
            x * self.interval + self.offset for x in range(1, (self.board_size + 1))
        ]
        ...
        for loc in self.location:
            self.line(
                (loc, self.offset + self.interval),
                (loc, 500 - self.interval),
                color=GRID_COLOR,
            )
            self.line(
                (self.offset + self.interval, loc),
                (500 - self.interval, loc),
                color=GRID_COLOR,
            )
        ...
        for idx_x, _ in enumerate(self.location):
            self.board.create_text(
                (
                    self.location[0] + idx_x * self.interval,
                    self.location[0] - idx_offset,
                ),
                text=idx_x,
                width=5,
                fill="gray",
        ...
        if random_selection:
            ...
            map = pd.read_csv(
                os.path.join("./board_library", map_info), index_col="Unnamed: 0"
            )
            self.whole_points = [
                point for point in self.whole_points if map.loc[point[1]][point[0]]
            ]
        ...
        # Initiate Machine
        self.machine = MACHINE(whole_points=self.whole_points, location=self.location)
    ...
    def line(self, start, end, color):
        self.last_line = self.board.create_line(
            start[0], start[1], end[0], end[1], fill=color, width=LINE_WIDTH
        )

    def occupy_triangle(self, triangle, color=USER_COLOR):
        if self.turn == "USER":
            self.last_triangle = self.board.create_polygon(
                [self.offset + self.interval * (x + 1) for x in list(chain(*triangle))],
                fill=color,
                outline=LINE_COLOR,
                width=2,
            )
    ...
    def user_go(self):
        ...
        if self.check_availability("USER", line):
            ...
            draw = [
                (self.location[point[0]], self.location[point[1]]) for point in line
            ]
